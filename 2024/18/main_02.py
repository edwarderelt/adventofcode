...
while path_found == True:
    ...
    print("Bytes to fall:", bytes_to_fall)
    # for row in matrix:
    #     print("".join(row))
    ...
    def explore(explored: set, i: int, j: int, direction: str, previous_cost: int) -> int:
        ...
        global lowest_costs
        if global_lowest_cost != float("inf"):
            return
    ...
    if global_lowest_cost == float("inf"):
        print("No path found, bytes_to_fall", bytes_to_fall)
        print("final_byte: ", lines[bytes_to_fall-1])
        path_found = False
    else:
        print("Lowest cost:", global_lowest_cost)
#176632
#171600
#168596
#90460